...
class TestListCategoriesUseCaseImpl:
    _scrape_book_repository_mock: Mock
    _use_case_input: ListCategoriesUseCaseInput
    _use_case_output_mock: Mock
    _use_case: ListCategoriesUseCaseImpl
    ...
    async def test_execute_use_case_successfully(self) -> None:
        # arrange
        books = ScrapeBooks()
        books.append(ScrapeBookFaker.fake(category="01"))
        books.append(ScrapeBookFaker.fake(category="02"))
        ...
        self._use_case_output_mock.success.assert_called_once_with(books.categories)
