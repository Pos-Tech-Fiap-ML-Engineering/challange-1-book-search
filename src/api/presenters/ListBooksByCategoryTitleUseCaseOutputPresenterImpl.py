...
class ListBooksByCategoryTitleUseCaseOutputPresenterImpl(
    BasePresenter, ListBooksByCategoryTitleUseCaseOutputHandler
):

    def success(self, result: list[ScrapeBook]) -> None:
        self._set_result(
            JSONResponse(status_code=200, content=BookOutput.to_output_list_json(result))
        )
