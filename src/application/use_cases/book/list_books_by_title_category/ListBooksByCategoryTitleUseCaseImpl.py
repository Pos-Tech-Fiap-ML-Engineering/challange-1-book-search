...
class ListBooksByCategoryTitleUseCaseImpl(
    ...
    async def execute_async(self, use_case_input: ListBooksByCategoryTitleUseCaseInput,
                            use_case_output: ListBooksByCategoryTitleUseCaseOutputHandler) -> None:
        books = await self._scrape_book_repository.get_all_books_async()

        books_result = books.list_books_by_category_or_title(title=use_case_input.title,
                                                             category=use_case_input.category)
